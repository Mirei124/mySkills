...
export type GraphNode = { id: string; type: "task" | "knowledge"; title: string; status?: string; claim_kind?: string; progress?: number | null; path?: string; summary?: string; markdown?: string; metadata?: Record<string, unknown> };
...
function App() {
  ...
  const filteredNodes = useMemo(() => nodes.filter((node) => {
    ...
    if (progressFilter === "unknown" && (node.type !== "task" || progress != null)) return false;
    if (!["all", "unknown"].includes(progressFilter) && (node.type !== "task" || progress == null)) return false;
    if (progressFilter === "not-started" && progress !== 0) return false;
    if (progressFilter === "in-progress" && (progress == null || progress <= 0 || progress >= 100)) return false;
  ...
  return <main>
    <header className="topbar">
      ...
      <div className="filters" aria-label="Filters"><label><span className="sr-only">Status</span><select aria-label="Status" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setFocused(undefined); }}><option value="all">Status · All</option>{statuses.map((status) => <option key={status} value={status}>{status.replaceAll("_", " ")}</option>)}</select></label><label><span className="sr-only">Progress</span><select aria-label="Progress" value={progressFilter} onChange={(event) => { setProgressFilter(event.target.value); setFocused(undefined); }}><option value="all">Progress · All</option><option value="unknown">Unknown</option><option value="not-started">0%</option><option value="in-progress">1–99%</option><option value="complete">100% estimate</option></select></label><label><span className="sr-only">Type</span><select aria-label="Type" value={typeFilter} onChange={(event) => { setTypeFilter(event.target.value); setFocused(undefined); }}><option value="all">Type · All</option><option value="task">Tasks</option><option value="knowledge">Knowledge</option></select></label><button className="reset-filters" disabled={!filtersActive} onClick={resetFilters} aria-label="Reset filters" title="Reset filters and focus">↺ Reset</button></div>
